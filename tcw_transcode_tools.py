...
def ffmpeg_audio_transcode(audio_input, output_path_body, just_output_file_name, codec, quality_or_bitrate, audio_channels):
    ...
    if codec == "opus":
        stream = ffmpeg.output(input_stream, full_output_path, ac=audio_channels, audio_bitrate=quality_or_bitrate)
        ...
        stream = ffmpeg.output(input_stream, full_output_path, ac=audio_channels, q=quality_or_bitrate)
    else:
        print("\n\n\ncodec not supported\n\n\n")
